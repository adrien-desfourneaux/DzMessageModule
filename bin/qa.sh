...
checkdoc () {
    ...
    find src view \( -name "*.php" -or -name "*.phtml" \) | xargs grep -E -sL "@category\s+${Category}" | awk '{print "Mauvaise catégorie : "$1}'
    find src view \( -name "*.php" -or -name "*.phtml" \) | xargs grep -E -sL "@package\s+${Package}" | awk '{print "Mauvais nom de package : "$1}'
    find src view \( -name "*.php" -or -name "*.phtml" \) | xargs grep -E -sL "@author\s+${Author}" | awk '{print "Mauvais auteur : "$1}'
    find src view \( -name "*.php" -or -name "*.phtml" \) | xargs grep -E -sL "@license\s+${License}" | awk '{print "Mauvaise licence : "$1}'
    find src view \( -name "*.php" -or -name "*.phtml" \) | xargs grep -E -sL "@link\s+${Link}" | awk '{print "Mauvais lien : "$1}'
}
...
genclassmap () {
    ...
    ../../vendor/bin/classmap_generator.php --library src/DzMessageModule --output ./autoload_classmap.php --overwrite --sort .
}
